...
OUT = ROOT / "output" / "pdf" / "知华科技-ZhuaTech-VMS-产品使用手册-V1.1.pdf"
...
def build():
    ...
    doc = ManualDoc(str(OUT), pagesize=letter, leftMargin=1 * inch, rightMargin=1 * inch,
                    topMargin=.72 * inch, bottomMargin=.68 * inch, title="知华科技 ZhuaTech VMS 产品使用手册 V1.1",
    ...
    story += [Spacer(1, 1.15 * inch), P("ZHUATECH  ·  PRODUCT GUIDE", "center"), Spacer(1, 12),
              ...
              Spacer(1, 25), callout("从预约到离场的可追踪闭环", "适用于园区前台、行政接待、内部接待人、安保人员和系统管理员。本手册以社区源码版 V1.1 实际功能为准。"),
              Spacer(1, 28), P("版本 V1.1  ·  2026 年 8 月", "center"),
    ...
    story += [P("审批或驳回", "h2"), P("在预约列表点击“处理”。核对访客、接待人、时间、区域和事由后，选择“通过并发码”或“驳回”。审批通过后系统签发六位数字通行码。"),
              ...
              bullet("查询预约编号或手机后四位，确认访客与预约记录一致。"),
    ...
    story += page("7. 基础设置与权限", "MODULE 05")
    story += [P("基础设置决定审批方式、通行码有效范围、消息渠道和数据留存期限，仅系统管理员可修改。"),
              screenshot("vms-settings.png", "图 7-1  基础设置：园区审批、数据通知和演示账号"),
    ...
    story += page("8. 移动工作台", "MOBILE WORKSPACE")
    ...
    story += [P("移动端面向接待人、前台和访客。首页聚合接待任务、待审批、提醒和风险关注，并提供四个高频入口。"), mobile, P("快捷服务", "h2"),
              data_table(["入口", "用户可以完成"], [
                  ["发起预约", "填写访客、接待、时间、区域和事由"], ["访客签到", "扫描预约码或输入编号，进入身份核验"],
                  ["通行码", "查看数字码、有效时间、区域和接待人"], ["异常上报", "记录异常、等级、关联预约和责任人"],
                  ["事项 / 消息", "查看待办、接待提醒和风险通知"],
              ], [1.3, 5.2])]

    story += page("9. 移动端操作说明", "MOBILE PROCEDURES")
    ...
    story += page("10. 准入风险判断", "RISK RULES")
    ...
    story += page("11. 数据、隐私与安全", "DATA & PRIVACY")
    ...
    story += page("13. 安装与体验", "INSTALLATION")
    ...
    story += page("14. 许可边界与技术支持", "SUPPORT")
